<?php
	...
	class WS_Form_API_Helper extends WS_Form_API {
		...
		// API - Debug
		public function api_debug($parameters) {

			// Check supplied debug console state
			if(
				!isset($parameters['helper_debug']) ||
				!in_array($parameters['helper_debug'], array('off', 'administrator', 'on'))
			) {
				return array('error' => true, 'error_message' => __('Invalid debug state', 'ws-form'));
			}

			// Set debug console state
			WS_Form_Common::option_set('helper_debug', $parameters['helper_debug']);
