<?php
	...
	class WS_Form_Action_User_ACF extends WS_Form_Action_User {
		...
		public function __construct() {
			...
			add_filter('wsf_action_user_form_actions', array($this, 'hook_form_actions'), 10, 4);
			add_filter('wsf_action_user_form_meta', array($this, 'hook_form_meta'), 10, 3);

			// Form submitting
			add_filter('wsf_action_user_field_mapping', array($this, 'hook_field_mapping'), 10, 6);
			add_action('wsf_action_user_file', array($this, 'hook_file'), 10, 2);
			add_action('wsf_action_user_attachments', array($this, 'hook_attachments'), 10, 0);
			add_action('wsf_action_user_user_meta', array($this, 'hook_user_meta'), 10, 2);
		...
		public function hook_action_settings($settings) {
			...
			array_splice($settings['meta_keys'], 7, 0, 'action_user_acf_validation');
			array_splice($settings['meta_keys'], 7, 0, 'action_user_field_mapping_acf');
		...
		public function hook_list_fields($list_fields, $list_id) {
			...
			// Get fields
			$fields = WS_Form_ACF::acf_get_fields_all(array('user_type' => $list_id), false, true, false, false);
		...
		public function hook_form_meta($form_meta, $form_field_id_lookup_all, $list_id) {
			...
			$fields = WS_Form_ACF::acf_get_fields_all(array('user_type' => $list_id), false, false, true, false);
		...
		public function hook_field_mapping($field_mapping_return, $form, $submit_parse, $config, $deduplication_mapping, $list_id) {
			...
			foreach($field_mapping_acf as $field_map_acf) {
				...
				$get_submit_value_repeatable_return = parent::get_submit_value_repeatable($submit_parse, $field_name, array(), true);
				...
				if(!parent::duplication_repeatable_return($get_submit_value_repeatable_return, $field_id, $deduplication_mapping)) {
		...
		public function hook_attachments() {
			...
			foreach($this->acf_attachments as $acf_attachment) {
				...
				$acf_parent_field_type = $acf_attachment['acf_parent_field_type'];
		...
		// Process user meta
		public function hook_user_meta($user_id, $list_id) {
			...
			foreach($this->acf_update_fields as $acf_key => $meta_value) {
				...
				update_field($acf_key, $meta_value, $user_id);
			}
		...
		public function hook_get($return_array, $form, $user_id, $list_id) {
			...
			if(is_array($field_mapping_acf)) {
				...
				$acf_field_data = WS_Form_ACF::acf_get_field_data($user_id);
		...
		public function hook_svg_custom_field_logos($svg_custom_field_logos, $list_id) {
			...
			if(WS_Form_ACF::acf_get_fields_all(array('user_type' => $list_id), false, true, false, true)) {
