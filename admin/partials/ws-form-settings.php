...
<script>
	...
	(function($) {
		...
		$(function() {
			...
			// Send test submit error email
			$('[data-action="wsf-submit-error-test"]').on('click', function() {

				wsf_obj.submit_error_test(function() {

					// Show info message
					wsf_obj.message('<?php esc_html_e('Form submission error email successfully sent!', 'ws-form'); ?>', true, 'notice-success');
					...
					// Show info message
					wsf_obj.message('<?php esc_html_e('An error occurred while sending the form submission error email. Confirm the recipient email address is correct.', 'ws-form'); ?>', true, 'notice-error');
...
<?php
					...
					case 'wsf-submit-error-test' :
?>
<input type="button" class="wsf-button wsf-button-inline" data-action="wsf-submit-error-test" data-for="wsf_<?php WS_Form_Common::echo_esc_attr($field); ?>" value="<?php esc_attr_e('Send Test', 'ws-form'); ?>"<?php WS_Form_Common::attributes_e($attributes); ?> />
<?php
						break;
					default :
