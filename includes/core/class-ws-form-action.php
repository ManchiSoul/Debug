<?php
	...
	abstract class WS_Form_Action {
		...
		public function process_return_actions($actions, $form, $submit) {

			// Check actions
			if(!is_array($actions)) { return self::error(__('Return actions should be an array', 'ws-form')); }

			// Process actions
			foreach($actions as $action => $action_params) {

				// Get action from inner array to allow multiple actions to run
				$action = self::get_action_param($action_params, 'action', $action);
				...
				switch($action) {
					...
					case 'redirect' :
						...
						if($url !== false) {
							...
							// Redirect to URL
							self::success(sprintf(__('Redirect added to queue: %s', 'ws-form'), $url), array(
					...
					case 'message' :
						...
						if($message !== false) {
							...
							// Show the message
							self::success(sprintf(__('Message added to queue: %s', 'ws-form'), $message), array(
								...
								array(
									...
									'message_hide' => self::get_action_param($action_params, 'message_hide', '')
								)
							));
						}
					...
					case 'error' :
						...
						self::error(self::get_action_param($action_params, 'message', ''));
					...
					case 'field_value' :
						...
						$field_id = absint(self::get_action_param($action_params, 'field_id'));

						if($field_id > 0) {

							self::success(sprintf(__('Set field value added to queue for field ID: %u', 'ws-form'), $field_id), array(

								array(

									'action' => 'field_value',
									'field_id' => $field_id,
									'section_repeatable_index' => self::get_action_param($action_params, 'section_repeatable_index', 0),
									'value' => self::get_action_param($action_params, 'value', ''),
									'check' => self::get_action_param($action_params, 'check', true),
									'append' => self::get_action_param($action_params, 'append'),
									'prepend' => self::get_action_param($action_params, 'prepend')
								)
							));
						}
					...
					case 'cookie_set' :
						...
						$name = self::get_action_param($action_params, 'name');
						$value = self::get_action_param($action_params, 'value');

						if(!empty($name)) {

							WS_Form_Common::cookie_set($name, $value, true, false, '');

							self::success(sprintf(__('Set cookie: %s', 'ws-form'), $name));
						}
		...
		public static function get_action_param($action_params, $param, $default_value = false) {
			...
			if(is_object($action_params)) {

				return property_exists($action_params, $param) ? $action_params->{$param} : $default_value;
			}
